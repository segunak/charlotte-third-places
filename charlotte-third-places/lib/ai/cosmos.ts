...
/**
 * Perform vector similarity search on chunks filtered to specific places.
 * This is the "two-phase" approach: first find relevant places, then search
 * only their partitions for chunks. Much faster than cross-partition search.
 * 
 * @param queryEmbedding - The embedding vector to search with (1536 dimensions)
 * @param topK - Maximum number of results to return
 * @param minScore - Minimum similarity score threshold (0-1)
 * @param placeIds - Array of place IDs to search within
 * @returns Array of chunk documents with similarity scores, ordered by relevance
 */
export async function vectorSearchChunksForPlaces(
  queryEmbedding: number[],
  topK: number = 10,
  minScore: number = 0.7,
  placeIds: string[]
  ...
  // Build IN clause for place IDs
  // Cosmos DB doesn't support array parameters directly in IN, so we build the query dynamically
  const placeIdParams = placeIds.map((_, i) => `@placeId${i}`).join(", ");
  const placeIdParameters = placeIds.map((id, i) => ({ name: `@placeId${i}`, value: id }));
  ...
  const query = `
    SELECT TOP @topK
      ...
      VectorDistance(c.embedding, @queryEmbedding) AS distance
    FROM c
    WHERE c.placeId IN (${placeIdParams})
      AND VectorDistance(c.embedding, @queryEmbedding) < @maxDistance
    ORDER BY VectorDistance(c.embedding, @queryEmbedding)
  `;
